...
namespace xRoadMap.Module.Controllers
{
    ...
    public partial class cspraWindowController : WindowController
    {
            ...
            where T : IEvento
        {
            ...
            TipoGeometriaEvento tipoGeometria;
            if (t.GetType().IsSubclassOf(typeof(EventoLineare)))
            {
                ...
                tLin.Km = RoutingHelper.GetChilometricaFromMeasure(tLin as IEventoOnRoad, tLin.M);
                tLin.KmFine = RoutingHelper.GetChilometricaFromMeasure(tLin as IEventoOnRoad,tLin.MFine);
            ...
            else
            {
                ...
                tPoint.Km = RoutingHelper.GetChilometricaFromMeasure(tPoint as IEventoOnRoad,tPoint.M);
                RoutingHelper.UpdatePointCoordinate(tPoint);
            }

            if (tipoLocalizzazione == TipoPosizione.Coordinate)
            {
                if (tipoGeometria == TipoGeometriaEvento.Lineare)
                {
                ...
                else if (tipoGeometria == TipoGeometriaEvento.Puntuale)
                {
